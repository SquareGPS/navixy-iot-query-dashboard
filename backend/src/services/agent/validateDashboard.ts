 ...
 * here are cross-field semantic checks (id uniqueness across row children,
 * grid-rect overlap, re-running the live SQL guard), not shape assertions.
...
export function validateDashboard(schema: unknown): DashboardValidation {
  ...
  if (Array.isArray(panels)) {
    panels.forEach((panel: unknown, index: number) => {
      ...
      // id breaks state isolation between panels.
      ...
      // Row children participate in id uniqueness (collapsed rows carry
      // nested panels); per-panel shape rules stay on the top level.
      if (type === 'row' && Array.isArray(panel.panels)) {
        panel.panels.forEach((child: unknown, childIndex: number) => {
          if (isPlainObject(child) && child.id !== undefined && child.id !== null) {
            recordId(child.id, `${path}.panels[${childIndex}]`);
          }
        });
      }
      ...
      // text panels short-circuit before the SQL fetch and row panels are
      // never rendered directly — neither carries a statement by design.
      if (type === 'text' || type === 'row') return;
